...
contract StarNFTV2 is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function mintNFT(address to, uint64 level, uint64 starRating, uint64 computingPower, string memory quality, string memory color) public returns (uint256) {
        _tokenIds.increment();

        uint64 newNFTId = uint64(_tokenIds.current()) ; 
        _safeMint(_staking, newNFTId);

        nfts[newNFTId] = NFT(newNFTId, level, starRating, computingPower, quality, color);

        ISTAKING(_staking).stak(to,newNFTId);
